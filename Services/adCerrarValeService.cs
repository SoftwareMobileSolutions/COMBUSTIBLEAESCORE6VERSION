...
{
    public class adCerrarValeService : IadCerrarVale
    {
        private readonly conexion conexion;

        public adCerrarValeService(conexion _conexion)
        {
            conexion = _conexion;
        }
        ...
        public async Task<IEnumerable<rpValesCerradosModel>> ObtenerValesCerrados(int UserID, int CompanyID, string FechaIncio, string fechaFin)
        {
            IEnumerable<rpValesCerradosModel> data = null;
            string sp = "EXEC SP_ObtenerValesCerrados @UserID,@CompanyID,@FechaIncio,@fechaFin";
            var con = new SqlConnection(conexion.Value);
            try
            {
                if (con.State == ConnectionState.Closed)
        ...
        public async Task<IEnumerable<rpValesCerradosXPlacaModel>> ObtenerValesCerradosXPlaca(int UserID, int CompanyID, string Placa, string FechaIncio, string fechaFin)
        {
            IEnumerable<rpValesCerradosXPlacaModel> data = null;
            string sp = "EXEC SP_ObtenerValesCerradosXPlaca @UserID,@CompanyID,@Placa,@FechaIncio,@fechaFin";
            var con = new SqlConnection(conexion.Value);
            try
            {
                if (con.State == ConnectionState.Closed)
